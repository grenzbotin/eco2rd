...
import { DATA_CENTER_GREEN, DATA_CENTER_RED, DATA_CENTER_UNKNOWN } from "config/datacenterTypes";
  ...
  return (
    <div className={`datacenter ${variant}`}>
      ...
      <div>
        <b>{renderedText}</b>
        <span>
          (last updated: {timestamp ? new Date(timestamp).toLocaleDateString() : "unknown"})
        </span>
      </div>
    </div>
  );
